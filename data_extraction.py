...
class DataExtractor:
    ...
    Attributes:
        database_connector (DatabaseConnector): the instance of DatabaseConnector.
        engine (Engine): the engine of the SQLAlchemy application.
        headers (dict): the HTTP header with the x-api-key for authentication.
    '''
    ...
    def list_db_tables(self) -> list:
        '''This method returns a list of table names from the database engine.'''
        return self.engine.table_names()
    
    def read_rds_table(self, table_name: str) -> DataFrame:
        '''
        This method converts the RDS (AWS Relational Database Service) table to a dataframe.
    ...
    def list_number_of_stores(self, number_of_stores_endpoint: str) -> int:
        '''
        This method is used to get the number of stores from the endpoint.

        Args:
            number_of_stores_endpoint (str): the endpoint to the number of stores
    ...
    def retrieve_stores_data(self, retrieve_a_store_endpoint: str, number_of_stores: int) -> DataFrame:
        '''
        This method is used to collect individual store data in a list which is converted to a dataframe.

        Args: 
            retrieve_a_store_endpoint (str): the endpoint to store_details
    ...
    def extract_from_s3(self, s3_address: str) -> DataFrame:
        '''
        This method is used to get the object from the AWS S3 address which is converted to a dataframe.
