...
class DataTransforms:
    '''
    This class contains static helper methods that clean dataframes or columns.
    '''
    @staticmethod
    def remove_null_values(df: DataFrame) -> DataFrame:
        '''
        This static method is used to remove null values from the ``df`` dataframe.
    ...
    def remove_alphanumerical_values(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove alphanumerical values from the ``column_name`` dataframe.
    ...
    def remove_non_alpha_values(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove non-alphabetical values from the ``column_name`` dataframe.
    ...
    def remove_values_with_non_alpha_prefix(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove values with a non-alphabetical prefix from the ``column_name`` dataframe.

        Valid prefixes are 2 characters long, or 3 if they denote web stores.
    ...
    def remove_strings_of_undesired_length(df: DataFrame, column_name: str, desired_length: int) -> DataFrame:
        '''
        This static method is used to remove strings of undesired length from the ``column_name`` dataframe.
    ...
    def remove_non_digit_values(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove non-digit values from the ``column_name`` dataframe.

        it removes values with alphabetical characters, whitespaces, etc and keeps values with digits.
    ...
    def remove_non_datetime_values(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove non-datetime values from the ``column_name`` dataframe.
        ...
        Examples:
            ...
            ''
        '''
        is_datetime = lambda x: type(pd.to_datetime(x, errors='coerce')) 
        return df[column_name].apply(lambda x: x if is_datetime(x)== Timestamp else '')
    
    @staticmethod
    def format_datetime_values(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to format datetime values in the ``column_name`` dataframe.
    ...
    def remove_non_store_types(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove non-store_type values from the ``column_name`` dataframe.
    ...
    def remove_invalid_emails(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove invalid email addresses from the ``column_name`` dataframe.
        ...
        return df[column_name].apply(lambda x: x if re.fullmatch(pattern, x) else '')
    
    @staticmethod
    def remove_non_countries(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove non-countries from the ``column_name`` dataframe.
    ...
    def correct_gb_country_code(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to correct 'GB' country code typos in the ``column_name`` dataframe.
    ...
    def remove_non_card_providers(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove non-card-providers from the ``column_name`` dataframe.
    ...
    def remove_strings_with_numbers(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove strings with numbers from the ``column_name`` dataframe.

        It removes values with digits and keeps values with alphabetical characters, whitespaces, etc.
    ...
    def remove_strings_with_invald_weight_suffix(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove strings with invalid weight suffix from the ``column_name`` dataframe.
    ...
    def calculate_weight_multiples(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to calculate the total weights of each row in the ``column_name`` dataframe.
        ...
        df[column_name] = df[column_name].apply(
            lambda x: str(float(x.split('x')[0]) * float(x.split('x')[-1][:-2])) + x[-2]
                      if re.search(PATTERN_KG_ML, x) else x)
        df[column_name] = df[column_name].apply(
            lambda x: str(float(x.split('x')[0]) * float(x.split('x')[-1][:-1])) + x[-1]
    ...
    def convert_product_weights_to_kg(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to convert the weights to floats in kg in the ``column_name`` dataframe.
        ...
        df[column_name] = df[column_name].apply(
            lambda x: str(float(x[:-1]) / 1_000 if re.search(PATTERN_G, x) else x))
        df[column_name] = df[column_name].apply(
            lambda x: str(float(x[:-2]) / 1_000 if re.search(PATTERN_ML, x) else x))
    ...
    def remove_invalid_product_statuses(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove invalud product statuses from the ``column_name`` dataframe.
    ...
    def remove_non_time_periods(df: DataFrame, column_name: str) -> DataFrame:
        '''
        This static method is used to remove non-time-periods from the ``column_name`` dataframe.
