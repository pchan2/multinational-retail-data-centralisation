...
class DataCleaning(DataTransforms):
    ...
    def clean_user_data(self, df: DataFrame) -> DataFrame:
        ...
        Returns:
            df (DataFrame): the dataframe of legacy_users.
        '''
        df['date_of_birth'] = self.remove_non_datetime_values(
            df, 'date_of_birth')
        ...
        df['phone_number'] = self.remove_alphanumerical_values(
            df, 'phone_number')
    ...
    def clean_card_data(self, df: DataFrame) -> DataFrame:
        '''
        This method is used to clean the card_details dataframe from
        card_details.pdf.

        The card_details dataframe is populated with the
        DatabaseConnector.retrieve_pdf_data method.
        ...
        Returns:
            df (DataFrame): the dataframe of card_details.
        '''
        df['card_provider'] = self.remove_non_card_providers(
            df, 'card_provider')
        df['date_payment_confirmed'] = self.remove_non_datetime_values(
            df, 'date_payment_confirmed')
        df['date_payment_confirmed'] = self.format_datetime_values(
            df, 'date_payment_confirmed')
        df['expiry_date'] = self.remove_alphanumerical_values(
            df, 'expiry_date')
    ...
    def clean_store_data(self, df: DataFrame) -> DataFrame:
        '''
        This method is used to clean the store_details dataframe from
        the store_details endpoint.

        The store_details dataframe is populated with the
        DatabaseConnector.retrieve_stores_data method.
        ...
        df['opening_date'] = self.remove_non_datetime_values(
            df, 'opening_date')
        ...
        df['country_code'] = self.remove_strings_of_undesired_length(
            df, 'country_code', 2)
    ...
    def convert_product_weights(self, df: DataFrame) -> DataFrame:
        '''
        This method is used to clean the weight column from the
        products.csv S3 address.

        The products dataframe is populated with the
        DatabaseConnector.extract_from_s3 method.
    ...
    def clean_orders_data(self, df: DataFrame) -> DataFrame:
        ...
        Returns:
            df (DataFrame): the dataframe of orders_table.
        '''
        df = df.drop(['level_0', 'index', 'first_name',
                     'last_name', '1'], axis=1)
        df = df.drop_duplicates()
        df = self.remove_null_values(df)
        return df
